const odd_dirs = [{"x": 1, "y": 0}, {"x": 1, "y": -1}, {"x": 0, "y": -1}, {"x": -1, "y": 0}, {"x": 0, "y": 1}, {"x": 1, "y": 1}];
const even_dirs = [{"x": 1, "y": 0}, {"x": 0, "y": -1}, {"x": -1, "y": -1}, {"x": -1, "y": 0}, {"x": -1, "y": 1}, {"x": 0, "y": 1}];


class HexDemoScene extends Phaser.Scene {
    constructor() {
      super({ key: 'HexDemoScene' })
    }
    ...
    create() {
        console.log("This is working, fuck yeah")
        gameState.map = this.add.tilemap('map');

        gameState.tileset = map.addTilesetImage('tileset', 'tiles');
    
        gameState.map.createLayer('Calque 1', tileset);
    
        gameState.cursors = this.input.keyboard.createCursorKeys();
    
        gameState.camera = this.cameras.main;
        gameState.camera.setZoom(0.45);
        gameState.camera.centerOn(0, 0);
        ...
        gameState.controls = new Phaser.Cameras.Controls.SmoothedKeyControl(controlConfig);

        //marker = this.add.sprite(0, 0, 'outline2');
        gameState.marker2 = this.add.image(0, 0, 'outline3');

        gameState.hex_matching = this.cache.json.get('hex_matching');

        gameState.wave= init_wavefunction();

        for (var i = 0; i < gameState.map.layers[0].data.length; i++) {
            for (var j = 0; j < gameState.map.layers[0].data[i].length; j++) {
                gameState.map.layers[0].data[i][j].index = 1;
            }
        }
        ...
        gameState.hexagonWidth = 48;
        gameState.hexagonHeight = 51;
        gameState.sectorWidth = 48;
        gameState.sectorHeight = 37.5;
        gameState.gradient = 1/2;
    }
    ...
    checkMatching(hex_matching, x, y) {
    ...
    get_min_entropy_coords(wavefunction) {
        min_entropy = 100;
        min_entropy_options = [];
    ...
    collapse_wavefunction_to_value(wavefunction, coords, value) {
        wavefunction[coords.y][coords.x] = [value];
    }

    propagate_wavefunction(wavefunction, coords, only_walls = false) {
        var stack = [];
        stack.push(coords);
    ...
    constrain_wavefunction(wavefunction, coords, option) {
    ...
    get_possible_neighbors(superposition, direction_index, only_walls = false) {
    ...
    iterate_wavefunction_controlled(wavefunction, coords) {
        collapse_wavefunction(wavefunction, coords);
        propagate_wavefunction(wavefunction, coords);
    }

    is_wavefunction_collapsed(wavefunction) {
        for (var i = 0; i < wavefunction.length; i++) {
            for (var j = 0; j < wavefunction[i].length; j++) {
                var entropy = wavefunction[i][j].length;
                if (entropy > 1) {
    ...
    run_wavefunction_collapse() {
        var iteration = 1;
        var wave= init_wavefunction();
    ...
    fill_rooms_below_size(wavefunction, size) {
    ...
    fill_rooms_below_width(wavefunction, size) {
    ...
    remove_walls_below_size(wavefunction, size) {
        var rooms = get_rooms(wavefunction, 0, 63);
        for (var i = 0; i < rooms.length; i++) {
    ...
    remove_walls_below_width(wavefunction, size) {
        var rooms = get_rooms(wavefunction, 0, 63);
        console.log()
        for (var i = 0; i < rooms.length; i++) {
            for (var x = 0; x < rooms[i].length; x++) {
    ...
    get_rooms(wavefunction, min_val, max_val) {
        var rooms = [];
    ...
    set_tile_map_with_wavefunction(wavefunction) {
        for (var i = 0; i < wavefunction.length; i++) {
            for (var j = 0; j < wavefunction[i].length; j++) {
